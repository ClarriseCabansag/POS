...
// Function to delete a cashier row with confirmation
function deleteRow(button) {
    const row = button.closest('tr');
    const cashierId = row.getAttribute('data-id');  // Get the cashier ID
    ...
    .then(data => {
        if (data.success) {
            ...
            row.remove();
            showNotification('Cashier deleted successfully'); // Use notification instead of alert
        } else {
    ...
    .catch(error => {
        console.error('Error:', error);
        showNotification('Error deleting cashier'); // Handle error with notification
    });
}
