document.addEventListener("DOMContentLoaded", function () {
    ...
    const btnClear = document.querySelector(".fas.fa-backspace").parentElement;
    ...
    // Handle the "x" button click (remove one digit at a time)
    btnClear.addEventListener("click", function () {
        if (tillAmount.length > 0) {
            tillAmount = tillAmount.slice(0, -1); // Remove the last character
            tillAmountDisplay.value = tillAmount; // Update the input box
        }
    });
